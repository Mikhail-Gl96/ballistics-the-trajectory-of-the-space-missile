...
rocket_m_stages_dry_arr = [rocket_m_1_stage_dry, rocket_m_2_stage_dry, rocket_m_3_stage_dry]
...
rocket_T_2_stage = 64
rocket_T_3_stage = 56
rocket_T_stages_arr = [rocket_T_1_stage, rocket_T_2_stage + rocket_T_1_stage, rocket_T_3_stage + rocket_T_2_stage]
...
def get_m_current(n_now_in):
    ...
    dm = m_per_sec(m_stage_start[n_now_in], m_stage_dry[n_now_in], time_work_max_stage[n_now_in])
    view_in_details(f'dm [{slo(dm)}] = m_per_sec(m_stage_start[n] [{slo(m_stage_start[n_now_in])}], '
                    f'm_stage_dry[n] [{slo(m_stage_dry[n_now_in])}], '
                    f'time_work_max_stage[n] [{slo(time_work_max_stage[n_now_in])}])')
    ...
    for n in range(stage_numbers):
        ...
        if flag is True:
            ...
            if next_stage[n] is True:
                ...
                break
            print(f'theta normal = {theta} rad   theta earth= {THETA_EARTH} rad     phi= {phi} rad    h= {h} m     '
                  f'pitch = {pitch} rad   alpha = {alpha} rad  ')
    break
...
graphs_out = ['alpha', 'theta', 'THETA_EARTH', 'pitch', 'plot V', 'h', 'V(r)']
